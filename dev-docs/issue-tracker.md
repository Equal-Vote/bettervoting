...
**Note:** `Equal-Vote/bettervoting` is the canonical repo where issues and PRs should live — this holds regardless of what git remote name(s) a given contributor's clone uses (`origin`, `upstream`, or otherwise). `gh`'s auto-detection may resolve to the wrong remote, so always pass `--repo Equal-Vote/bettervoting` explicitly on `gh issue`/`gh pr` commands rather than relying on remote-based inference.
