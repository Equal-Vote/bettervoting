import { PartialBy } from "./Election";
import { Uid } from "./Uid";
...
export interface ElectionRoll {
    ...
    registration?: unknown; // LEGACY: only set by the dormant registerVoterController; never read.
    precinct?: string; // Precint of voter
    email_data?: {
        // SendGrid's response shape varies (single response vs. array of responses) depending on the code path that sets it
...
// Input type for callers building a roll to be inserted. create_date / update_date / head
// are generated by the model inside the write transaction — callers must not set them.
// Use the ElectionRoll returned by the write to read those values back.
export type NewElectionRoll = PartialBy<ElectionRoll,'create_date'|'update_date'|'head'>;
