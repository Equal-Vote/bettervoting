...
export default function makeApp() {
    ...
    // 500kb (up from the 100kb default) so bulk uploads of voters/ballots fit in a reasonably sized batch
    ...
    app.get('*', (req, res) => {
        fs.readFile(path.join(__dirname, frontendPath, req.url.split('?')[0]), 'utf8', (err) => {
            if(err){
