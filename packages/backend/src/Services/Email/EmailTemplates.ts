...
// Transactional mail: no `asm` group. A voter who unsubscribed from the campaign
// group must still be able to get their own voter id back, and SendGrid suppresses
// anything sent under a group the address has opted out of.
const transactionalSettings: Partial<Imsg> = {
    mail_settings: emailSettings.mail_settings,
}

// Deliberately link-free: the voter id is the credential, and the voter types it
// into the election page themselves. No vote URL, no election URL.
export function VoterIdReminder(election: Election, email: string, voter_ids: string[]): Imsg {
    const term = election.settings.term_type ?? 'election';
    const title = sanitizeText(election.title);
    const idsText = voter_ids.join(', ');
    const idsHtml = voter_ids.map(id => `<p style="font-size:20px"><b>${sanitizeText(id)}</b></p>`).join('');
    const testNotice = election.state === 'draft';
    return {
        ...transactionalSettings,
        to: email,
        from: process.env.FROM_EMAIL_ADDRESS ?? '',
        subject: `Your voter ID for ${election.title}`,
        text: `${testNotice ? `[⚠️Test ${term}] ` : ''}Someone asked us to send the voter ID for this address in the ${term} "${election.title}". `
            + `Your voter ID is: ${idsText}. Enter it on the ${term} page to vote. `
            + `If you did not ask for this, you can ignore this email.`,
        html: emailTemplate(`
          ${testNotice ? `<h3>⚠️This ${term} is still in test mode. All ballots during test mode will be removed once the election is finalized, and at that time you will need to vote again.⚠️</h3>` : ''}
          <p>Someone asked us to send the voter ID for this address in the ${term} "${title}".</p>
          <p>Your voter ID${voter_ids.length > 1 ? 's are' : ' is'}:</p>
          ${idsHtml}
          <p>Enter it on the ${term} page to vote. This ID is unique to you; please don't share it.</p>
...
// unsubscribe: false renders the footer without the SendGrid group-unsubscribe tag.
// Use it for transactional mail that is sent without `asm`, where the tag would have
// nothing to resolve to.
function emailTemplate(content: string, { unsubscribe = true }: { unsubscribe?: boolean } = {}) {
  const footer = unsubscribe ? '<%asm_group_unsubscribe_url%>' : '';
