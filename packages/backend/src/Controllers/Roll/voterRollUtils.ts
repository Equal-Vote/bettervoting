...
export async function getOrCreateElectionRoll(req: IRequest, election: Election, ctx: ILoggingContext, voter_id_override?: string, skipStateCheck?: boolean): Promise<ElectionRoll | null> {
    ...
    if (electionRollEntries == null) {
        ...
        // Only persist the identifying fields this election actually authenticates on —
        // i.e. the same values the lookup above ran against. Writing req.user.email /
        // ip_hash unconditionally attached a real email address and IP hash to the
        // voter's ballot_id even for elections configured to require neither.
        ...
        if ((auth_ip_hash || email || voter_id)) {
            ...
            // Not persisted *here*, but the cast-vote path does reach this branch and
            // CastVoteStore.submitBallotEvent inserts whatever roll it is handed — so this
            // row does land in electionRollDB for e.g. open_open elections. Keep it free of
            // anything the election didn't ask to authenticate on.
