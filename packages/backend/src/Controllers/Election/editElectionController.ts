...
const editElection = async (req: IElectionRequest, res: Response, next: NextFunction) => {
    ...
    const expected_update_date = inputElection.update_date as string;
    if (typeof expected_update_date !== 'string') {
        throw new BadRequest("Election payload missing update_date version token");
    }
